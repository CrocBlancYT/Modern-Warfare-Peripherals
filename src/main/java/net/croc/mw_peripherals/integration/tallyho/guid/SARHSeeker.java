...
public class SARHSeeker extends GuidanceComponent {
    ...
    public record SeekerProperties(int seeker_fov, int seeker_range, int lead_coefficient, float max_G) {
        public void appendHoverText(List<Component> components) {
                components.add(Component.literal("Guidance: SARH")
                        .setStyle(Style.EMPTY.withColor(ChatFormatting.AQUA)));

                components.add(Component.literal(String.format("Fov: %s°", this.seeker_fov))
                        .setStyle(Style.EMPTY.withColor(ChatFormatting.WHITE)));

                components.add(Component.literal(String.format("Seeker Range: %sm", this.seeker_range))
                        .setStyle(Style.EMPTY.withColor(ChatFormatting.WHITE)));
        }
    }


    public static class Factory implements IGuidanceFactory<SARHSeeker> {
        private final SARHSeeker.SeekerProperties props;

        public Factory(int seeker_fov, int seeker_range, int lead_coefficient, float max_G) {
            this.props = new SARHSeeker.SeekerProperties(seeker_fov, seeker_range, lead_coefficient, max_G);
        }

        public SARHSeeker create() {
            return new SARHSeeker(this.props);
        }
    ...
    private SARHSeeker(SeekerProperties props) {
