...
public class IRSeekerGround extends GuidanceComponent {
    ...
    private IRSeekerGround(SeekerProperties properties) {
        ...
        if (!missile.level().isClientSide) {
            if (this.isSeeking()) {
                ...
                if (t != null && t.isAlive()) {
                    ...
                    Optional<Ship> ship = tryLock(missile.level(), lookDir, missile.position(), (AABB)null, fov, this.properties.seeker_range());
    ...
    public static record SeekerProperties(int seeker_fov, int gimbal_fov, double seeker_fov_rate, int lead_coefficient, float max_G, int seeker_range, float decoy_resistance) {
        public void appendHoverText(List<Component> components) {
            components.add(Component.literal("Guidance: IR, All-Aspect (Ground)").setStyle(Style.EMPTY.withColor(ChatFormatting.AQUA)));
    ...
    public static class Factory implements IGuidanceFactory<IRSeekerGround> {
        ...
        public IRSeekerGround create() {
            return new IRSeekerGround(this.props);
        }
